...
{
    if (const std::partial_ordering order{ x <=> other_.x }; order != 0) return order;
    if (const std::partial_ordering order{ y <=> other_.y }; order != 0) return order;
    if (const std::partial_ordering order{ z <=> other_.z }; order != 0) return order;
    return w <=> other_.w;
}
