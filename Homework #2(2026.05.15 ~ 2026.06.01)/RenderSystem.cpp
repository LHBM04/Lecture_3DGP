...
Microsoft::WRL::ComPtr<ID3D12RootSignature> RenderSystem::uiRootSignature;
Microsoft::WRL::ComPtr<ID3D12PipelineState> RenderSystem::uiPipelineState;
std::unordered_map<std::size_t, Microsoft::WRL::ComPtr<ID3D12PipelineState>> RenderSystem::meshMaterialPipelineStates;
std::unordered_map<std::size_t, Microsoft::WRL::ComPtr<ID3D12PipelineState>> RenderSystem::uiMaterialPipelineStates;
...
bool RenderSystem::CreateUIPipeline()
{
	D3D12_ROOT_SIGNATURE_DESC rootSignatureDesc{};
...
ID3D12PipelineState* RenderSystem::GetMeshPipelineState(const Material& material_)
{
...
ID3D12PipelineState* RenderSystem::GetUIPipelineState(const Material& material_)
{
	const Shader* const vertexShader{ material_.GetVertexShader() };
	const Shader* const pixelShader{ material_.GetPixelShader() };
	if (nullptr == vertexShader || nullptr == pixelShader || !vertexShader->HasByteCode() || !pixelShader->HasByteCode())
...
bool RenderSystem::CreateMeshPipelineState(
...
bool RenderSystem::CreateUIPipelineState(
	const D3D12_SHADER_BYTECODE& vertexShader_,
	const D3D12_SHADER_BYTECODE& pixelShader_,
	ID3D12PipelineState** pipelineState_)
{
