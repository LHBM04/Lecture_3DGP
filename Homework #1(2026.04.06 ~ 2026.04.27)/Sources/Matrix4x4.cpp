...
Matrix4x4::Matrix4x4(float value_)
	: XMFLOAT4X4(
...
Matrix4x4::Matrix4x4(const float values_[16])
	: XMFLOAT4X4(
...
Matrix4x4::Matrix4x4(Matrix matrix_)
{
	Store(*this, matrix_);
}

Matrix4x4::Matrix4x4(const Matrix4x4& other_)
	: XMFLOAT4X4(other_)
{
}
...
Matrix4x4 Matrix4x4::operator*(const Matrix4x4& other_) const
{
	return Matrix4x4(DirectX::XMMatrixMultiply(Load(*this), Load(other_)));
}

Matrix4x4& Matrix4x4::operator*=(const Matrix4x4& other_)
{
...
bool Matrix4x4::operator==(const Matrix4x4& other_) const
{
...
std::partial_ordering Matrix4x4::operator<=>(const Matrix4x4& other_) const
{
	const float* lhs{ &_11 };
...
float Matrix4x4::operator[](size_t index_) const
{
	return (&_11)[index_];
}

float& Matrix4x4::operator[](size_t index_)
{
	return (&_11)[index_];
}
...
bool Matrix4x4::TryGetInverse(Matrix4x4& result_) const
{
	Vector determinant;
	const Matrix inverse{ DirectX::XMMatrixInverse(&determinant, Load(*this)) };
...
bool Matrix4x4::CanInverse() const
{
	return std::abs(GetDeterminant()) > Mathf::EPSILON;
}

Vector3D Matrix4x4::MultiplyPoint3x4(const Vector3D& point_) const
{
	Vector3D result;
...
Vector3D Matrix4x4::MultiplyPoint(const Vector3D& point_) const
{
	Vector3D result;
...
Vector3D Matrix4x4::MultiplyVector(const Vector3D& vector_) const
{
	Vector3D result;
...
Vector3D Matrix4x4::GetWorldPosition() const
{
...
Vector3D Matrix4x4::GetLossyScale() const
{
	return GetScale();
}

Vector4D Matrix4x4::GetRow(std::size_t index_) const
{
	const float* base{ &_11 };
	return Vector4D(base[index_ * 4 + 0], base[index_ * 4 + 1], base[index_ * 4 + 2], base[index_ * 4 + 3]);
}

Vector4D Matrix4x4::GetColumn(std::size_t index_) const
{
	switch (index_)
	{
...
void Matrix4x4::SetRow(std::size_t index_, const Vector4D& row_)
{
	float* base{ &_11 };
...
void Matrix4x4::SetColumn(std::size_t index_, const Vector4D& column_)
{
	switch (index_)
	{
...
bool Matrix4x4::IsValidTRS() const
{
	// Minimal TRS validity check: affine last row and finite components.
...
bool Matrix4x4::IsIdentity(float epsilon_) const
{
	return IsApproximately(*this, GetIdentity(), epsilon_);
}

bool Matrix4x4::IsApproximately(const Matrix4x4& lhs_, const Matrix4x4& rhs_, float epsilon_)
{
...
void Matrix4x4::SetTRS(const Vector3D& position_, const Quaternion& rotation_, const Vector3D& scale_)
{
...
Matrix4x4 Matrix4x4::Translate(const Vector3D& translation_)
{
	return Matrix4x4(DirectX::XMMatrixTranslation(translation_.x, translation_.y, translation_.z));
}

Matrix4x4 Matrix4x4::Rotate(const Quaternion& rotation_)
{
	return Matrix4x4(DirectX::XMMatrixRotationQuaternion(Quaternion::Load(rotation_)));
}

Matrix4x4 Matrix4x4::Scale(const Vector3D& scale_)
{
	return Matrix4x4(DirectX::XMMatrixScaling(scale_.x, scale_.y, scale_.z));
}

Matrix4x4 Matrix4x4::TRS(const Vector3D& translation_, const Quaternion& rotation_, const Vector3D& scale_)
{
	return Scale(scale_) * Rotate(rotation_) * Translate(translation_);
}

Matrix4x4 Matrix4x4::LookAt(const Vector3D& from_, const Vector3D& to_, const Vector3D& up_)
{
	return Matrix4x4(DirectX::XMMatrixLookAtLH(Vector3D::Load(from_), Vector3D::Load(to_), Vector3D::Load(up_)));
}

Matrix4x4 Matrix4x4::Perspective(float fovYDegrees_, float aspect_, float nearZ_, float farZ_)
{
	return Matrix4x4(DirectX::XMMatrixPerspectiveFovLH(fovYDegrees_ * Mathf::DEG2RAD, aspect_, nearZ_, farZ_));
}

Matrix4x4 Matrix4x4::Ortho(float left_, float right_, float bottom_, float top_, float nearZ_, float farZ_)
{
	return Matrix4x4(DirectX::XMMatrixOrthographicOffCenterLH(left_, right_, bottom_, top_, nearZ_, farZ_));
}

Matrix4x4 Matrix4x4::Ortho(float width_, float height_, float nearZ_, float farZ_)
{
	return Matrix4x4(DirectX::XMMatrixOrthographicLH(width_, height_, nearZ_, farZ_));
}

Matrix4x4 Matrix4x4::Frustum(float left_, float right_, float bottom_, float top_, float nearZ_, float farZ_)
{
	return Matrix4x4(DirectX::XMMatrixPerspectiveOffCenterLH(left_, right_, bottom_, top_, nearZ_, farZ_));
}

bool Matrix4x4::TryInverse3DAffine(const Matrix4x4& input_, Matrix4x4& result_)
{
	Vector determinant;
	const Matrix inverse{ DirectX::XMMatrixInverse(&determinant, Load(input_)) };
	if (DirectX::XMVector4Equal(determinant, DirectX::XMVectorZero()))
	{
		return false;
	}
...
Matrix Matrix4x4::Load(const Matrix4x4& matrix_)
{
	return DirectX::XMLoadFloat4x4(&matrix_);
}

void Matrix4x4::Store(Matrix4x4& destination_, Matrix source_)
{
